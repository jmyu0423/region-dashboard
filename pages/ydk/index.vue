...
<script>
import { changshumyeon, byeonggokmyeon, younghaemyeon, chuksanmyeon, jipummyeon, dalsanmyeon, namjungmyeon, ganggumyeon, youngdukeup } from './geojson';
...
export default {
  ...
  data(){
    ...
    currDay: '',
    map: null,
    changshumyeon: changshumyeon,
    byeonggokmyeon: byeonggokmyeon,
    younghaemyeon: younghaemyeon,
    chuksanmyeon: chuksanmyeon,
    jipummyeon: jipummyeon,
    dalsanmyeon: dalsanmyeon,
    namjungmyeon: namjungmyeon,
    ganggumyeon: ganggumyeon,
    youngdukeup: youngdukeup,
    customWindow:{
      width: '100%',
    ...
    defaultBjdongCd: "4777000000", //영덕군
  ...
  methods:{
    ...
    setGeoLayout(map){
        ...
        map.data.addGeoJson(this.changshumyeon, true);
        map.data.addGeoJson(this.byeonggokmyeon, true);
        map.data.addGeoJson(this.younghaemyeon, true);
        map.data.addGeoJson(this.chuksanmyeon, true);
        map.data.addGeoJson(this.jipummyeon, true);
        map.data.addGeoJson(this.dalsanmyeon, true);
        map.data.addGeoJson(this.namjungmyeon, true);
        map.data.addGeoJson(this.ganggumyeon, true);
        map.data.addGeoJson(this.youngdukeup, true);
    ...
    setDetail(map){
      for(let i=0; i<map.data._features.length; i++){
        ...
        if(feature.property_EMD_NM === "병곡면"){
          x = feature.bounds._max.x;
          y = feature.bounds._max.y + 0.02
        }else if(feature.property_EMD_NM === "영해면"){
          x = feature.bounds._max.x;
          y = feature.bounds._max.y + 0.01
        }else if(feature.property_EMD_NM === "영덕읍"){
          x = feature.bounds._max.x;
          y = feature.bounds._max.y - 0.02;
        }else if(feature.property_EMD_NM === "강구면"){
          x = feature.bounds._max.x - 0.01;
          y = ((feature.bounds._min.y + feature.bounds._max.y) / 2) - 0.03;
        }else if(feature.property_EMD_NM === "남정면"){
          x = feature.bounds._min.x - 0.02;
          y = (feature.bounds._min.y + feature.bounds._max.y) / 2;
        }else if(feature.property_EMD_NM === "지품면"){
          y = feature.bounds._max.y + 0.04
        }else if(feature.property_EMD_NM === "축산면"){
          y = feature.bounds._max.y + 0.03
        }else if(feature.property_EMD_NM === "달산면"){
          y = feature.bounds._max.y + 0.02
        }
    ...
    initMap(){
        let mapOptions={
            center: new naver.maps.LatLng(36.45507989999999, 129.3459681)
            , zoom: 11
    ...
    setSelectAllList(){
      let tempBjdongList = [];
      tempBjdongList.push(changshumyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(byeonggokmyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(younghaemyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(chuksanmyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(jipummyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(dalsanmyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(namjungmyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(ganggumyeon.features[0].properties.EMD_CD + "00");
      tempBjdongList.push(youngdukeup.features[0].properties.EMD_CD + "00");
